...
def ndf_to_rawsco(ndf):
    ...
    if abs(clock - 1789773) < 2:
        ...
        fc_rate_4 = 200.0
        fc_rate_5 = 160.0
    fc_rate_4_inv = 1.0 / fc_rate_4
    fc_rate_5_inv = 1.0 / fc_rate_5
    ndf = ndf[1:]
            ...
            for ch in ["p1", "p2"]:
                if ch_to_se[ch]:
                    ...
                    if ch_to_swe_divider[ch] <= 0:
                        ...
                        if (
                            ch_to_timer[ch] >= 8
                            and ch_to_stimer[ch] < 0x800
                            and ch_to_ss[ch] > 0
                        ):
                            ch_to_timer[ch] = (
                                0 if ch_to_stimer[ch] < 0 else ch_to_stimer[ch]
                            )
...
def rawsco_to_ndf(rawsco):
    ...
    ndf = [
        ...
        (
            ...
            2,
            1,
        ),  # This is necessary to prevent channel silence for low notes
        ("apu", "p2", "du", 0, 3, 0),
        ("apu", "p2", "lh", 1, 3, 0),
        ("apu", "p2", "cv", 1, 3, 0),
        ("apu", "p2", "vo", 0, 3, 0),
