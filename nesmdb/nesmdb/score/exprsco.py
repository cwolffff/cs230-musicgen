...
def exprsco_downsample(exprsco, rate, adaptive):
    ...
    assert abs(rate_prev - 44100.0) < 1e-6
    if abs(rate - 44100.0) < 1e-6:
    ...
    if rate is None:
        ...
        intervals_t = {i / 44100.0: c for i, c in list(intervals.items())}

        # Raise error if we don't have enough info to estimate tempo
        num_intervals = sum(intervals.values())
        if num_intervals < 10:
            raise Exception(
                "Too few intervals ({}) to estimate tempo".format(num_intervals)
            )
        ...
        disc = 0.001
        rate_errors = []
        for rate in np.arange(1.0, 24.0 + disc, disc):
            error = 0.0
...
def exprsco_to_rawsco(exprsco, clock=1789773.0):
