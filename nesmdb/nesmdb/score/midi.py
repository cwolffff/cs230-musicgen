...
def exprsco_to_midi(exprsco):
    ...
    for i, ins in enumerate([p1, p2, tr, no]):
        for (start_samp, note, velocity), end_samp in notes[i]:
            ...
            start_t, end_t = start_samp / 44100.0, end_samp / 44100.0
            note = pretty_midi.Note(
                velocity=velocity, pitch=note, start=start_t, end=end_t
            )
            ins.notes.append(note)

        for samp, cc_num, arg in ccs[i]:
            cc = pretty_midi.ControlChange(cc_num, arg, samp / 44100.0)
    ...
    eos = pretty_midi.TimeSignature(1, 1, nsamps / 44100.0)
